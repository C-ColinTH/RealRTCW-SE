...
static qboolean offset_ext = qfalse;
#ifdef USE_VOIP
...
#define VOICE_AHEAD       0.02f // The time to the next rendering
#define VOICE_TIMESHIFT   0.1f  // Adjust the timing
#define VOICE_WINDOW      0.004535147f // (100/22050) Window of the moving average
#define VOICE_BOUNDARY    0.05f  // 
#define VOICE_PREFIX      0.1f  // Prefixed silence
#define VOICE_POSTFIX     0.1f  // Postfixed silence
#define VOICE_SCALE       0.5f  // Scale the average
#define VOICE_SHORT       0 // Length of short voice files
#define VOICE_IMPULSE     48   // Voice amplitude for short voice files
#define VOICE_CUTOFF      25    // Remove rippling in the lips movement ?
#define VOICE_NOISE       5000  // Only samples above this value is consider
#define VOICE_EXPONENT    0.25f // The exponent to apply to the voice amplitude
#define VOICE_FACTOR      1.5f  // The factor to apply to the voice amplitude

/*
   =================
   S_AL_VoiceFree

   Free the memory allocated for the sound data.
   =================
   */

static void S_AL_VoiceFree(void** p){
  if(p && *p){
    Z_Free(*p);
    *p = NULL;
  }
...
/*
   ...
   Load the sound data from file and prefix and postfix it with silence. 
   =================
   */

static void* S_AL_VoiceLoad(const char* filename, snd_info_t* info, qboolean transform){
  ...
  original = (unsigned char*)S_CodecLoad(filename, info);

  if(original && (info->samples > 0)){

    data = info->samples * info->width * info->channels;

    if(transform){

      prefix = ((int)(VOICE_PREFIX * info->rate));
      postfix = ((int)(VOICE_POSTFIX * info->rate));
    ...
    info->samples += prefix;
    info->samples += postfix;

    prefix *= ( info->width * info->channels);
    postfix *= ( info->width * info->channels);

    info->size += prefix;
    info->size += postfix;

    transformed = (unsigned char*)Z_Malloc(info->size);

    if(transformed){
      if(data){
        memcpy(transformed +  prefix, original, data); 
      }
      if(info->width == 1){
        memset(transformed, 128, prefix);
        memset(transformed + prefix + data, 128, postfix);
      }
    }else{
      memset(info, 0, sizeof(snd_info_t));
    }

    Hunk_FreeTempMemory(original);
  }
  return transformed;  
}
/*
   ...
   Decodes the sound data into 16 bits stereo. 
   =================
   */

static void S_AL_VoiceDecode(snd_info_t* info,void* data, int index, short* left, short* right){
  ...
  *left = *right = 0;
  ...
  if(info->width == 1){
    ...
    u8 =  (unsigned char*)((byte*)data + (index * info->width * info->channels));

    if(info->channels >= 1) {
      *right = *left =  (u8[0] - 128) << 8;
    }
    if(info->channels >= 2) {
      *right = (u8[1] - 128) << 8;
    }
    ...
    s16 =  (short*)((byte*)data +  (index * info->width * info->channels));
...
/*
   =================
   S_AL_VoiceMovingAverage

    Calculate the moving average of the sound in a window.  
   =================
   */

static int S_AL_VoiceMovingAverage(snd_info_t* info,void* data, float time, float window ){
  ...
  if(data && info->samples){
    ...
    win =  (int)(window* info->rate);

    // Clamp the window within the sound file.
    
    if(0 == win){
      win = 1;
    }

    end = (int)( time  * info->rate);
    ...
    for( i = 0 ; i < win ; i++){
      ...
      S_AL_VoiceDecode(info, data, end - i , &left, &right);

      sample = left;

      if( abs(sample) > (VOICE_NOISE)){
         sum +=( (  sample * 255) >>  8) ;
      }
    }
    average = sum / win;
...
/*
  ...
  Calculate the amplitude of the sound (or rather the silence).
   =================
   */

static unsigned char S_AL_VoiceAmplitude(sfxHandle_t sfx, float time){
  ...
  if(curSfx->voice && base.samples ){

    prefix = ((int)(VOICE_PREFIX * base.rate));
    postfix = ((int)(VOICE_POSTFIX * base.rate));

    base.samples -= prefix;
    base.samples -= postfix;

    if(base.samples > 0){
    
      offset = time + VOICE_TIMESHIFT - VOICE_PREFIX;

      window = VOICE_WINDOW;

      total = ((float)base.samples) / base.rate;
      
      data = ((byte*)curSfx->voice) + (prefix * base.width * base.channels);

      if( (offset < 0 ) ||  (offset >= total)){
        window = VOICE_BOUNDARY;
      }
      average = S_AL_VoiceMovingAverage( &base, data, offset, window );

      average =  abs(average) ;

      // Compress
      average =  (int)(average * VOICE_SCALE);
         
      if(total < VOICE_SHORT){
        average =  VOICE_IMPULSE;
      }
...
static void S_AL_BufferUnload(sfxHandle_t sfx)
{
  ...
  if(knownSfx[sfx].voice){
    S_AL_VoiceFree(&knownSfx[sfx].voice);
  }
...
/*
   =================
   S_AL_BufferLoadVoice

   Load the modified voice sound into a buffer.
   =================
   */
static void S_AL_BufferLoadVoice(sfxHandle_t sfx){
  ...
  if((curSfx->voice) || (curSfx->isDefault))
    return;

  data = S_AL_VoiceLoad(curSfx->filename, &info, qtrue);
  ...
  if(!data) {
    ...
    Com_Printf( S_COLOR_RED "ERROR: Can't decode voice file %s\n", curSfx->filename);
    return;
  }
  ...
  // Create a buffer
  if (!S_AL_GenBuffers(1, &buffer, curSfx->filename))
  {
    if(!curSfx->inMemory){
      S_AL_BufferUseDefault(sfx);
    }
    S_AL_VoiceFree(&data);
    Com_Printf( S_COLOR_RED "ERROR: Can't generate buffer for voice file %s\n", curSfx->filename);
    return;
  }
  ...
  {
    // We have no data to buffer, so buffer silence
    qalBufferData(buffer, AL_FORMAT_MONO16, (void *)dummyData, 2, 22050);
  }
  else
  {
  ...
  while(error == AL_OUT_OF_MEMORY)
  {
    if( !S_AL_BufferEvict( ) )
    {
      ...
      S_AL_VoiceFree(&data);
      Com_Printf( S_COLOR_RED "ERROR: Out of memory filling voice buffer with %s\n", curSfx->filename);
      return;
    }
  ...
  if(error != AL_NO_ERROR)
  {
    ...
    S_AL_VoiceFree(&data);
    Com_Printf( S_COLOR_RED "ERROR: Can't fill voice buffer for %s - %s\n", 
        curSfx->filename, S_AL_ErrorMsg(error));
    return;
  }


  // Replace the openal buffer if the original sound is cached.
  if(curSfx->inMemory){
    qalDeleteBuffers(1, &curSfx->buffer);
    ...
    if(error == AL_INVALID_OPERATION){
      qalDeleteBuffers(1, &buffer);
      S_AL_VoiceFree(&data);
      Com_Printf( S_COLOR_RED "ERROR: Can't replace voice buffer for %s - %s\n",
        curSfx->filename, S_AL_ErrorMsg(error));
      return;
    }
  }
  curSfx->buffer = buffer;
  curSfx->voice = data;
  curSfx->info = info;
  curSfx->inMemory = qtrue;
}
...
/*
   ...
   =================
   */
static void S_AL_BufferUseVoice(sfxHandle_t sfx)
{
  if(knownSfx[sfx].filename[0] == '\0')
    return;

  if((!knownSfx[sfx].voice) && (!knownSfx[sfx].isDefault)){
    S_AL_BufferLoadVoice(sfx);
  }
  knownSfx[sfx].lastUsedTime = Sys_Milliseconds();
}
/*
...
qboolean S_AL_BufferInit( void )
{
  ...
  default_sfx = S_AL_BufferFind( "***DEFAULT***" );
    ...
    int entity, int channel, int flags, qboolean local)
{
  ...
  if(sfx >= 0)
  {
    ...
    if( (entity < MAX_CLIENTS) && (channel == CHAN_VOICE) ){
      S_AL_BufferUseVoice(sfx);
    }else{
      S_AL_BufferUse(sfx);
    }
...
static void S_AL_MainStartSound( vec3_t origin, int entnum, int entchannel, sfxHandle_t sfx, int flags )
{
  ...
  if ( (entnum >= 0 && entnum < MAX_CLIENTS) && (entchannel == CHAN_VOICE) )
  {
    if(offset_ext && knownSfx[sfx].voice){
      ...
      memset( s_entityTalkAmplitude, 0, sizeof( s_entityTalkAmplitude ) );
      s_entityTalkAmplitude[entnum] =  s_alTalkAnims->integer;
    }
  }
...
void S_AL_SrcUpdate( void )
{
  ...
  for(i = 0; i < srcCount; i++)
  {
    ...
    entityChannel = srcList[i].channel;
    curSource = &srcList[i];

    last = curSource->updated;
    curSource->updated = now;
    ...
    if ( ( entityNum >= 0 && entityNum < MAX_CLIENTS) && (entityChannel == CHAN_VOICE ))
    {
      if(offset_ext && knownSfx[curSource->sfx].voice ){
        ...
        if(AL_NO_ERROR == qalGetError()){

          ahead = VOICE_AHEAD;

          if(last && (now > last)){
            ahead = (float) ( now - last)/ 1000;
          }

          s_entityTalkAmplitude[entityNum] = S_AL_VoiceAmplitude(curSource->sfx, offset + ahead) ;
      ...
      memset( s_entityTalkAmplitude, 0, sizeof( s_entityTalkAmplitude ) );
      s_entityTalkAmplitude[entityNum] =  s_alTalkAnims->integer;
    }
  }
}
...
int S_AL_GetVoiceAmplitude( int entityNum ) {
  ...
  value = s_entityTalkAmplitude[entityNum];

  if(value > VOICE_CUTOFF) {
    value = (value - VOICE_CUTOFF) / ( 255 - VOICE_CUTOFF);
    
    value =  (pow(value, VOICE_EXPONENT) * (255 - VOICE_CUTOFF))  +  VOICE_CUTOFF;

    value =  value * VOICE_FACTOR;
