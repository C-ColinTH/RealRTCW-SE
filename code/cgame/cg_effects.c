...
//	vec3_t	dir;
	...
	if ( cent->currentState.aiChar == AICHAR_ZOMBIE || cent->currentState.aiChar == AICHAR_ZOMBIE_SURV 
		|| cent->currentState.aiChar == AICHAR_ZOMBIE_FLAME  || cent->currentState.aiChar == AICHAR_ZOMBIE_GHOST ) {
...
void CG_LaunchGib( centity_t *cent, vec3_t origin, vec3_t angles, vec3_t velocity, qhandle_t hModel, float sizeScale, int breakCount ) {
	...
	} else if ( ( cent->currentState.aiChar == AICHAR_ZOMBIE || cent->currentState.aiChar == AICHAR_ZOMBIE_SURV  || cent->currentState.aiChar == AICHAR_ZOMBIE_FLAME
		|| cent->currentState.aiChar == AICHAR_ZOMBIE_GHOST ) && IS_FLAMING_ZOMBIE( cent->currentState ) ) {
	...
	for ( gibIndex = 0, count = 0, foundtag = qtrue; foundtag && gibIndex < MAX_GIB_MODELS && gibTags[gibIndex]; gibIndex++ ) {
		...
		for ( tagIndex = 0; ( tagIndex = CG_GetOriginForTag( cent, re, gibTags[gibIndex], tagIndex, origin, axis ) ) >= 0; count++, tagIndex++ ) {
			...
			if ( ( cent->currentState.aiChar == AICHAR_ZOMBIE ) ||
			     ...
			     ( cent->currentState.aiChar == AICHAR_ZOMBIE_GHOST ) ||
			...
			for ( j = 0; j < MAXJUNCTIONS; j++ )
			{
				if ( !Q_stricmp( JunctiongibTags[j], ConnectTags[i] ) ) {
					if ( newjunction[j] == qtrue ) {
						...
						if ( cent->currentState.aiChar == AICHAR_ZOMBIE || cent->currentState.aiChar == AICHAR_ZOMBIE_FLAME || cent->currentState.aiChar == AICHAR_ZOMBIE_SURV  || cent->currentState.aiChar == AICHAR_ZOMBIE_GHOST ) {
...
unsigned int randtable[NUMRANDTABLE] =
{
...
static qboolean CG_SpawnSmokeSprite( centity_t *cent, float dist, vec4_t color ) {
    ...
    if( smokesprite ) {
        ...
        smokesprite->colour[0] = color[0];
        smokesprite->colour[1] = color[1];
        smokesprite->colour[2] = color[2];
        smokesprite->colour[3] = color[3];
...
void CG_RenderSmokeGrenadeSmoke( centity_t *cent, const weaponInfo_t *weapon, weapon_t wtype ) {
...
//			spritesNeeded = 40 - cent->miscTime; 
		...
		if( spritesNeeded == 1 ) {
			...
			// this is better
			if( !CG_SpawnSmokeSprite( cent, 0.f, smokeColor ) )
				// try again, just in case, so we don't get lots of gaps and remain quite constant
				CG_SpawnSmokeSprite( cent, 0.f, smokeColor );
		} else {
//			float lerpfrac = 1.0f / (float)spritesNeeded;
			...
			for( dtime = spritesNeeded * spawnrate; dtime > 0; dtime-=spawnrate ) {
				// this is theoretically fine, till the smokegrenade ends up in a solid
				//while( !CG_SpawnSmokeSprite( cent, lerp * cg.frametime * SMOKEBOMB_SMOKEVELOCITY, smokeColor ) );

				// this is better
				if( !CG_SpawnSmokeSprite( cent, lerp * cg.frametime * SMOKEBOMB_SMOKEVELOCITY, smokeColor ) )
					// try again, just in case, so we don't get lots of gaps and remain quite constant
					CG_SpawnSmokeSprite( cent, lerp * cg.frametime * SMOKEBOMB_SMOKEVELOCITY, smokeColor );
			}
		}
